from ag95.DuckDbDatabase.DuckDbWrapper import (DuckColumnDef,
                                               DuckDbWrapper)
    ...
    def __init__(self,
        ...
        if not all_tables_def:
            self.all_tables_def = [{'table_name': 'my_db_table_name',
                                     'columns_def': [DuckColumnDef(column_name='my_column_name1',
                                                               column_type='INTEGER'),
                                                     DuckColumnDef(column_name='my_column_name2',
                                                               column_type='INTEGER')]}]
        else:
            self.all_tables_def = all_tables_def

    def migrate(self):
        with DuckDbWrapper(database_path=self.database_path) as DB:
